...
class SQLiteRepo:
    ...
    async def create_memory(self, m: Dict[str, Any]) -> Dict[str, Any]:
        with self._get_conn() as conn:
            conn.execute(
                """
                INSERT INTO memories (id, title, description, file_path, file_type, year, month, reactions, comments, created_at, is_deleted, is_favorite, tags, location_name)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    m["id"],
    ...
    async def list_memories(self, year: Optional[int] = None, month: Optional[int] = None, search: Optional[str] = None, favorites_only: bool = False) -> List[Dict[str, Any]]:
    ...
    async def delete_memory(self, memory_id: str) -> bool:
        with self._get_conn() as conn:
            cursor = conn.execute("UPDATE memories SET is_deleted = 1 WHERE id = ?", (memory_id,))
            if cursor.rowcount > 0:
                conn.execute("DELETE FROM covers WHERE memory_id = ?", (memory_id,))
            conn.commit()
            return cursor.rowcount > 0

    async def toggle_favorite(self, memory_id: str) -> Optional[Dict[str, Any]]:
        with self._get_conn() as conn:
            conn.execute("UPDATE memories SET is_favorite = 1 - is_favorite WHERE id = ?", (memory_id,))
            conn.commit()
        return await self.get_memory(memory_id)

    async def add_reaction(self, memory_id: str, r_type: str) -> Optional[Dict[str, Any]]:
        ...
        with self._get_conn() as conn:
            conn.execute("UPDATE memories SET reactions = ? WHERE id = ?", (json.dumps(reactions), memory_id))
            conn.commit()
        m["reactions"] = reactions
        return m

    async def add_comment(self, memory_id: str, comment: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        m = await self.get_memory(memory_id)
        if not m:
            return None
        comments = m["comments"]
        ...
        with self._get_conn() as conn:
            conn.execute("UPDATE memories SET comments = ? WHERE id = ?", (json.dumps(comments), memory_id))
            conn.commit()
        m["comments"] = comments
        return m
    ...
    async def get_covers(self) -> Dict[str, str]:
        with self._get_conn() as conn:
            rows = conn.execute("SELECT * FROM covers").fetchall()
            return {r["year_month"]: r["memory_id"] for r in rows}

    async def set_cover(self, year_month: str, memory_id: str) -> bool:
        with self._get_conn() as conn:
            conn.execute("""
                INSERT INTO covers (year_month, memory_id) 
                VALUES (?, ?) 
                ON CONFLICT(year_month) DO UPDATE SET memory_id = excluded.memory_id
            """, (year_month, memory_id))
            conn.commit()
...
class MongoRepo:
    ...
    async def get_memory(self, memory_id: str) -> Optional[Dict[str, Any]]:
        m = await self.memories.find_one({"id": memory_id})
        if m:
            m.pop("_id", None)
        return m
    ...
    async def delete_memory(self, memory_id: str) -> bool:
        res = await self.memories.update_one({"id": memory_id}, {"$set": {"is_deleted": True}})
        if res.modified_count > 0:
            await self.covers.delete_one({"memory_id": memory_id})
